 ...
 * (functionality untouched; only layout / Tailwind classes changed for mobile-right alignment)
 */
      ...
      <div className="max-w-5xl mx-auto bg-white p-4 rounded shadow">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2 items-center">
            <button onClick={() => { localStorage.removeItem("merchant_token"); navigate("/merchant-login"); }} className="px-3 py-1 bg-red-500 text-white rounded text-sm">Logout</button>
            <button onClick={() => loadShop()} className="px-3 py-1 bg-gray-200 rounded text-sm">Refresh</button>
          </div>
        </div>
          ...
          <div className="max-w-lg">
            ...
            <form onSubmit={saveShop} className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => { if (shop) setShopForm({ name: shop.name || "", phone: shop.phone || "", address: shop.address || "", pincode: shop.pincode || "" }) }} className="px-4 py-2 bg-gray-200 rounded text-sm">Reset</button>
              </div>
          ...
          <>
            ...
            <form onSubmit={submitItem} className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2 space-y-2">
                ...
                <div className="flex gap-2">
                  <button type="submit" className="px-3 py-1 bg-green-600 text-white rounded text-sm">{itemForm._editingId ? "Save" : "Add"}</button>
            ...
            <div className="space-y-3">
              {menu.length === 0 ? <div className="text-sm text-gray-500">No menu items</div> : menu.map(it => (
                // NOTE: changed layout classes to align everything to the right on mobile,
                // and revert to normal two-column on md+ screens.
                <div
                  key={it._id}
                  className="bg-white p-3 rounded border flex flex-col md:flex-row md:justify-between md:items-center items-end"
                >
                  <div className="w-full md:w-auto md:text-left text-right">
                        ...
                        <ul className="ml-4 list-disc">
                          {it.variants.map((v, idx) => (
                            <li key={idx} className="text-sm text-gray-700">
                              {v.label} — ₹{v.price} {v.available === false && <span className="text-xs text-red-600"> (disabled)</span>}
                            </li>
                          ))}
                        </ul>
                      </div>
            ...
            {orders.length === 0 ? <div className="text-sm text-gray-500">No orders</div> : (
              <div className="space-y-3">
                {orders.map(o => {
                  ...
                  return (
                    <div key={o._id} className="bg-white p-3 rounded border flex flex-col md:flex-row md:justify-between md:items-center items-end">
                      <div className="w-full md:w-auto md:text-left text-right">
                      ...
                      <div className="mt-3 md:mt-0 flex flex-col items-end gap-2">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => updateOrderStatus(o._id, "accepted")} disabled={status !== "received"} className={`px-2 py-1 rounded text-sm ${status === "received" ? "bg-blue-600 text-white" : "bg-gray-200"}`}>Accept</button>
                          <button onClick={() => updateOrderStatus(o._id, "packed")} disabled={status !== "accepted"} className={`px-2 py-1 rounded text-sm ${status === "accepted" ? "bg-yellow-500 text-white" : "bg-gray-200"}`}>Packed</button>
                          <button onClick={() => updateOrderStatus(o._id, "out-for-delivery")} disabled={status !== "packed"} className={`px-2 py-1 rounded text-sm ${status === "packed" ? "bg-indigo-600 text-white" : "bg-gray-200"}`}>Out</button>
                          <button onClick={() => updateOrderStatus(o._id, "delivered")} disabled={status !== "out-for-delivery"} className={`px-2 py-1 rounded text-sm ${status === "out-for-delivery" ? "bg-green-600 text-white" : "bg-gray-200"}`}>Delivered</button>
                        </div>
                      </div>
                    </div>
                  );
